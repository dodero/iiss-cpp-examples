...
int functionFactorial ( std::future<int>& promiseOfFuture ){
	...
	return finalFactorial; //Devolución de un valor futuro (finalfactorial)
}
...
int main(){
	...
	std::future<int> promiseOfFuture = promise.get_future(); //La promesa nos proporcionará un futuro
    std::future<int> future = std::async(std::launch::async , 
			functionFactorial, std::ref(promiseOfFuture));  //Ejecución asincrona
